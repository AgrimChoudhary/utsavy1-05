...
-- Guests can only update or delete their own wishes
CREATE POLICY "Guests can manage their own wishes" ON wishes
  FOR UPDATE
  TO public
  USING (
    guest_id = (
...
-- Guests can only remove their own likes
CREATE POLICY "Guests can remove their own likes" ON wish_likes
  ...
  USING (
    guest_id = (
      SELECT id FROM guests 
      WHERE guests.id = wish_likes.guest_id
    )
  );

--Let's implement the complete Wishing Wall feature for your wedding invitation platform. I'll create the necessary database tables, backend functionality, and frontend components.
