...
CREATE POLICY "Guests can view likes" ON wish_likes
  FOR SELECT
  TO public
  USING (true);
...
CREATE POLICY "Guests can remove their own likes" ON wish_likes
  ...
  USING (
    ...
    guest_id IN (
      SELECT guests.id FROM guests
      WHERE guests.user_id = auth.uiLet me create a comprehensive implementation plan for integrating the Wishing Wall feature into your wedding invitation platform.
